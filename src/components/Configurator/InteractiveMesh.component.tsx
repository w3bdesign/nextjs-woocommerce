...
export default function InteractiveMesh({
  ...
  useFrame((state, delta) => {
    ...
    if (targetState.rotation) {
      ...
      meshRef.current.rotation.x += (targetRotation.x - meshRef.current.rotation.x) * lerpFactor;
      meshRef.current.rotation.y += (targetRotation.y - meshRef.current.rotation.y) * lerpFactor;
      meshRef.current.rotation.z += (targetRotation.z - meshRef.current.rotation.z) * lerpFactor;
    }
    
    // Interpolate position
    if (targetState.position) {
      const targetPosition = new Vector3(...targetState.position);
      meshRef.current.position.x += (targetPosition.x - meshRef.current.position.x) * lerpFactor;
      meshRef.current.position.y += (targetPosition.y - meshRef.current.position.y) * lerpFactor;
      meshRef.current.position.z += (targetPosition.z - meshRef.current.position.z) * lerpFactor;
    }
    
    // Interpolate scale
    if (targetState.scale !== undefined) {
      const targetScale = typeof targetState.scale === 'number' 
        ? targetState.scale 
